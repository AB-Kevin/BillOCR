...
- **Verification passes** (Intake setting, under "Flagging", default 3 total): after the primary read of an image, Intake resamples the same image that many more times at a **Check-pass temperature** (also under "Flagging", default 0.5) and flags any field where a resample disagrees with the primary read. Set passes to `1` to turn this off (today's single-read behavior, no extra cost); higher catches more but adds that many more model calls per image, so it's proportionally slower. Raising the temperature makes check passes vary more from the primary read — more disagreement flags, but noisier ones; it has no effect while passes is `1`. `pipeline/field_validation.py`'s `values_equivalent()` tolerates pure formatting differences (`"150.00"` vs `150.0`) so those don't produce noise.
